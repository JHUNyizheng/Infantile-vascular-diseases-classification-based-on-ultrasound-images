from .Build_multimodal_fuse_head import *
from .Build_backbone import *
from .Build_neck import *
from .Build_decoder import *


class IHVMNet(nn.Module):
    """
    Build IHVMNet model
    ...
    def __init__(self, opt):
        ...
        self.build_classify_head = nn.Sequential(
            nn.Linear(int(embed_dim * 2 ** (len(depths) - 1)), int(embed_dim * 2 ** (len(depths) - 1)) * 4),
            nn.BatchNorm1d(int(embed_dim * 2 ** (len(depths) - 1)) * 4),
            nn.GELU(),
            nn.Linear(int(embed_dim * 2 ** (len(depths) - 1)) * 4, int(embed_dim * 2 ** (len(depths) - 1)) * 4),
            nn.BatchNorm1d(int(embed_dim * 2 ** (len(depths) - 1)) * 4),
            nn.GELU(),
            nn.Linear(int(embed_dim * 2 ** (len(depths) - 1)) * 4, n_classes),
            nn.BatchNorm1d(n_classes),
        )
        self.build_box_head = Detect(nc=2, ch=([decoder_chans, decoder_chans, decoder_chans, decoder_chans]))
    ...
    def forward(self, image_gray, image_color):
        # x = x.to(torch.device('cuda:1'))
        x = [image_gray, image_color]

        # x, score = self.build_MSEs_AMM(x)
        ...
        class_pred = self.build_classify_head(torch.sum(x[-1].view(x[-1].shape[0], x[-1].shape[1], -1), dim=-1))

        x = self.build_neck(x)
        box_pred = self.build_box_head(x)
        ...
        #     return [x, aux_x]
        # else:
        # return box_pred, class_pred, score
