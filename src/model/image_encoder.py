...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.acupoint_embed = nn.Parameter(torch.zeros(1, 1, embed_dim))
        ...
        embed_dims, depths = cmnext_settings['B2']
        extra_depths = depths
        dpr = [x.item() for x in torch.linspace(0.05, 0.1, 3)]
        norm_cfg = dict(type='BN', requires_grad=True)

        # self.ffm =FeatureFusionModule(64,num_heads=8)
    ...
    def forward(self, x, prompt):
        ...
        # append cls token
        acupoint_embed = self.acupoint_embed + self.pos_embed[:, :1, :]
        acupoint_tokens = acupoint_embed.expand(x.shape[0], -1, -1)
        x = torch.cat((acupoint_tokens, x), dim=1)
