...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        # pred_distri, pred_scores, pred_feature_class = torch.cat([xi.view(feats[0].shape[0], self.no + 4, -1) for xi in feats], 2).split(
        #     (self.reg_max * 4, self.nc, 4), 1)

        pred_distri, pred_scores, pred_feature_class = torch.cat(
            [xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1)

        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()
        # pred_feature_class = pred_feature_class.permute(0, 2, 1).contiguous()
        ...
        #     pred_scores.detach().sigmoid(), pred_feature_class.detach().sigmoid(), (pred_bboxes.detach() * stride_tensor).type(gt_bboxes.dtype),
        ...
        # loss[3] = self.class_loss(pred_feature_class, target_feature_class.to(dtype)).sum() / target_scores_sum
