...
class DiseaseBox(Dataset):
    ...
    def __init__(self, phase, transform=None, opt=None):
        self.dir_dataset = '/home/dell/Project/IHNet/data/增强'
        # Determenistic "random" shuffle of the maps:

        self.phase = phase

        files = os.listdir(self.dir_dataset)

        self.picture_name = []
        self.json_name = []
        for file in files:
            if '.json' in file:
                self.json_name.append(file)
            else:
                self.picture_name.append(file)
        ...
        random.seed(42)
        random.shuffle(self.json_name)

        if phase == 'train':
            self.length = int(len(self.json_name) * 0.8)
            # self.length = int(len(self.datadir))
        elif phase == 'val':
            self.length = int(len(self.json_name) * 0.2)
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'val':
            index = int(len(self.json_name) * 0.8) + index


        data_json = self.json_name[index]

        with open(os.path.join(self.dir_dataset, data_json)) as file:
            data = json.load(file)

        print('OK')
        im_file_gray = os.path.join(self.dir_dataset, data_json[:-5]+'.jpg')
        ...
        phlebolith_coords = []
        phleboliths = data['shapes']
        for phlebolith in phleboliths:
            if phlebolith['label'] == 'phlebolith':
                phlebolith_coords.append(phlebolith)
        w = data['imageWidth']
        h = data['imageHeight']
        scale_w = w / 640
        scale_h = h / 640
        ...
        # np.empty([0, 4])np.array([np.array([0, 0, 0, 0])]),
            ...
            for i in range(len(phlebolith_coords)):
                bboxed.append(convert_box(np.array(phlebolith_coords[i]['points']).flatten(), scale_w, scale_h))
                cls.append(1)
                batch_idx.append(data_json[:-5]+'.jpg')
        ...
        #             phlebolith_coords[i][1] = 640 - phlebolith_coords[i][1]
        #     if color != None:
        ...
        #         for i in range(len(phlebolith_coords)):
        #             phlebolith_coords[i] = np.array(phlebolith_coords[i]).flatten()
        #             phlebolith_coords[i][0] = 640 - phlebolith_coords[i][0]
        #     if color != None:
